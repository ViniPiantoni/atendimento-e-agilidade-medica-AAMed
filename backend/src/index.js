...
io.on('connection', (socket) => {
	...
	socket.on('user_solicitation', (data) => {
		...
		data.hospital_ids.map((ids) => {
			...
			if (ownerSocket) {
				socket.to(ownerSocket).emit('aviso', { user, description, currentLocation }); //aqui os hospitais perto vão receber esse chamado
			}
		});
	});
	...
	socket.on('arrived_manually', (data) => {
		...
		if (ownerSocket) {
			socket.to(ownerSocket).emit('arrived__manually_mobile', { arrived_mobile: arrived });
		}
	});
